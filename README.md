...
You can build the firmware locally without pushing to GitHub:

```bash
# Build the Docker image (first time only)
make docker-build

# Build firmware — outputs .uf2 files to ./output/
make docker-run
```

The built firmware files will appear in the `output/` directory. Copy them to your keyboard's UF2 mass storage device to flash.
